...
export class DashboardService extends EnhancedDataService {
  ...
  getBusinessPartnersCount() {
    this.url = `${this.baseUrl}Dashboard/GetBusinessPartnersCount`;

    return this.http.get(this.url).pipe(
      map((response: any) => {
        console.log("Business Partners Count:", response);
        return response["result"];
      }),
      catchError((error) => {
        console.error("Error fetching business partners count:", error);
        return throwError(error);
      })
    );
  }
  ...
  getSalesStats(startDate: string, endDate: string) {
    this.url = `${this.baseUrl}Dashboard/GetSalesStats`;
