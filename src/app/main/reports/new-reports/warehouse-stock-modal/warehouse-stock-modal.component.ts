...
import { WarehouseStockReport, WarehouseStockParams } from "../../models/item-tracking.model";
...
@Component({
  ...
  styleUrls: ["./warehouse-stock-modal.component.css"],
})
export class WarehouseStockModalComponent implements OnInit {
  ...
  form: FormGroup;
  reportData: WarehouseStockReport[] = [];
  ...
  generateReport() {
    ...
    const params: WarehouseStockParams = {
    ...
    console.log('🔍 Generating Warehouse Stock report with params:', params);

    this.loading = true;
    this.reportsService.getWarehouseStockLedger(params).subscribe({
      next: (data) => {
        console.log('✅ Warehouse Stock report data received:', data);
      ...
      error: (error) => {
        console.error('❌ Error generating Warehouse Stock report:', error);
        this.loading = false;
        const errorMessage =
  ...
  downloadPDF() {
    ...
    const fileName = `Warehouse_Stock_Report_${new Date().toISOString().split("T")[0]}.pdf`;
    doc.save(fileName);
